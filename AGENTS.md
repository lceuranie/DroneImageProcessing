...
│   └── visualization/     ← viewer.html, pointcloud.html, odm_pointcloud.laz
├── src/
│   ├── __init__.py
│   ├── extract_metadata.py
│   ├── georeference.py
...
- `data/visualization/viewer.html` is the Folium 2D viewer generated by `src/viewer.py`.
- `data/visualization/pointcloud.html` is the standalone deck.gl 3D point cloud viewer.
- `data/visualization/odm_pointcloud.laz` is required by the 3D viewer and must be
  committed so GitHub Pages can serve it.

**Point cloud viewer notes:**

- OpenDroneMap exports LAS 1.4, while the `loaders.gl` LAS/LAZ reader tops out at 1.3.
  If parsing fails, consider converting to LAS 1.3 (or PLY) before publishing.
- Consumer-drone point clouds typically arrive with 8-bit RGBA already, so do not
  blindly downscale 16-bit color unless the attributes are truly 0-65535.
- Always recenter UTM-scale coordinates before rendering in the browser to avoid
  Float32 precision loss and black screens in deck.gl.

## 9. References (read these, don't re-derive)
